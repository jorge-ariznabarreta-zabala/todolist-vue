<template>
  <div class="row container-fluid" id="CardContainer">
    ...
    <div v-for="todo in listTodo" :key="id" class="card-body border p-3">
        <div>
          <input class="toggle m-3 " type="checkbox" v-model="todo.completed" @change="editTodo(todo)" />
          <input type="text" @blur="editTodo(todo)" autofocus v-if="todo.completed" class="w-75 card-text shadow p-3 mb-5 bg-body-tertiary rounded text-decoration-line-through" v-model="todo.todo" />
          <input type="text" @blur="editTodo(todo)" autofocus v-else class="w-75 card-text shadow p-3 mb-5 bg-body-tertiary rounded" v-model="todo.todo" />
